...
scribe_intg errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern scribe_intg line; /* Source code line numbers - defined in scanner.c */
 ...
 * -------------------------------------------------------------
 */
scribe_void printScannerError(scribe_strg fmt, ...);
scribe_void displayScanner(BufferPointer ptrBuffer);
scribe_long getScannerFilesize(scribe_strg fname);
